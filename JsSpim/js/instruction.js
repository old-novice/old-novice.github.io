...
worker.onmessage = (event) =>
    event.data.forEach((e, i) => {
        InstructionUtils.instructionList[i].instructionElement.innerHTML = e;
        const keywd =
            InstructionUtils.instructionList[i].instructionElement.querySelector('.hljs-keyword');
        if (typeof showMipsInstHint === 'function' && keywd) {
            keywd.addEventListener('mouseover', () => {
                showMipsInstHint(keywd);
            });
            keywd.addEventListener('mouseout', () => {
                hideMipsInstHint();
            });
        }
    });
...
class InstructionUtils {
    ...
    static toggleInstHint(showInstRef) {
        window.mispInstHintEnabled = showInstRef;
    }
    static toogleChtHint(showChtHint) {
        window.mispChtHintEnabled = showChtHint;
    }
